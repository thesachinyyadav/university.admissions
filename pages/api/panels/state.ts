...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<PanelStateResponse | { message: string }>
) {
  if (req.method !== 'GET') {
  ...
  if (error) {
    console.error('[panel state] DB error:', error);
    return res.status(500).json({ message: 'Failed to fetch panel state' });
  }
